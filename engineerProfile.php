<?php
    ...
    if(isset($_SESSION['uname']))
    {
       // echo $_SESSION['uname'] . ", you have successfully been logged in. <br>";
    	if( ($_SESSION['role'] == "Admin") || ($_SESSION['role'] == "Customer") || ($_SESSION['role'] == "Conductor")){
	...
	<nav class="navbar navbar-inverse">
	  <div class="container-fluid">
	    ...
	    <ul class="nav navbar-nav">
	      ...
	      <li><a href="http://cs3380.rnet.missouri.edu/~GROUP8/engineerLogs.php">Logs</a></li>
	    </ul>
	                        ...
	                        <div class="navbar-login">
	                            <div class="row">
	                                ...
	                                <div class="col-lg-8">
	                                    ...
	                                    <p class="text-center">
	                                        <a href="http://cs3380.rnet.missouri.edu/~GROUP8/engineerProfile.php" class="btn btn-primary btn-block btn-sm">View Profile</a>
	                                    </p>
	                                </div>
	                            </div>
	                        </div>
...
<br>
	...
	<form action="http://cs3380.rnet.missouri.edu/~GROUP8/engineerLogs.php" method="POST" style="display:inline;">
		<input class="btn btn-primary" type="submit" name="engineerLogs" value="View Online Logs" />
	</form>
	<hr>
...
<?php 
	...
	if(isset($_POST['update']) && validatePassword($mysqli=connectToDB(), $_SESSION['uname'], $_POST['current_psswd'])){
		...
		if(!empty($_POST['new_psswd'])){	//user is changing password
			...
			updateEmployeeTable($mysqli, $_SESSION['uname'], $_POST['fname'], $_POST['lname'], $_POST['new_psswd'], $_POST['role'], $_POST['status'], $_POST['rank'], $_POST['totalhrs']);
			//updateAuthenticationTable($mysqli, $_SESSION['uname'], $_POST['new_psswd'], 'Engineer');
		}
		else{		//user not changing password
			updateEmployeeTable($mysqli, $_SESSION['uname'], $_POST['fname'], $_POST['lname'], $_POST['current_psswd'], $_POST['role'], $_POST['status'], $_POST['rank'], $_POST['totalhrs']);
		}
		$success = 1;
	}
	else{
	...
	if(isset($_POST['chg_info']) || isset($_POST['update'])){
		...
		$sql = "SELECT user_id, first_name, last_name, role, status, rank, total_hours_traveling
...
?>
		<form action="" method="POST">
			...
			<input type='text' name="totalhrs"  value= <?= $row["total_hours_traveling"] ?> style="display:none;" />
			
			User ID: <?= $row["user_id"] ?> <br>
			Role: <?= $row["role"] ?><br>
			Status: <?php if($row["status"] == 1){echo "Active";}else{echo "Inactive";} ?><br>
			Rank: <?= $row["rank"] ?><br>
			Total Travel Hours: <?= $row["total_hours_traveling"] ?><br>
			First Name: 
...
if(isset($_POST['trainsAssigned']))
{
   $mysqli = connectToDB();
    ...
    $sql = "SELECT 	Train.train_ID AS 'Train ID #', 
			...
			FROM Train JOIN Assigned_Engineer ON (Assigned_Engineer.train_ID=Train.train_ID)
			WHERE (Assigned_Engineer.user_id=?);";
    $stmt =$mysqli->stmt_init();
    if( !$stmt->prepare($sql) )
    {
        echo "Exit";
